...
class Rule(object):
    def __init__(self, condition_num, condition=None, action=None):
        ...
        self.condition = condition  # 规则条件和采用动作
        self.action = action
...
class QuestionSet:
    ...
    @staticmethod
    def has_cooperation_num_question(word_objects):
        """
        2.2 演员A和演员B有多少部合作的电影
        :param word_objects:
        :return:
        """
        select = u"?x"
        ...
        if person1 is not None and person2 is not None:
            ...
            return SPARQL_COUNT_TEM.format(prefix=SPARQL_PREXIX,
                                          select=select,
                                          expression=e)
        else:
            return None
    ...
    @staticmethod
    def has_cooperation_ask_question(word_objects):
        """
        3.2 演员A和演员B合作过某电影吗？
        :param word_objects:
        :return:
        ...
        if person1 is not None and person2 is not None:
            e = u"?p1 :personName '{person1}'." \
                ...
                u"?p1 :hasActedIn ?m." \
                u"?p2 :hasActedIn ?m." \
                u"?m :movieTitle '{movie}'".format(person1=person1, person2=person2, movie=movie1)

            return SPARQL_ASK_TEM.format(prefix=SPARQL_PREXIX,
                                            expression=e)
        else:
            return None
    ...
    def has_compare_question(word_objects):
        ...
        if person is not None and number is not None:
            ...
            e = u"?p :personName '{person}'." \
                ...
                u"?m :movieTitle ?x." \
                u"?m :movieRating ?r." \
                u"filter(?r {mark} {number})".format(person=person, number=number,mark=keyword)
    ...
    @staticmethod
    def has_specific_type_movie_num_question(word_objects):
        """
        2.3 某演员演了几部某类型的电影
        :param word_objects:
        :return:
    ...
    @staticmethod
    def is_sometype_person_question(word_objects):
        """
        3.3 某演员演过某类型的电影吗？
        :param word_objects:
        :return:
        ...
        for w in word_objects:
            if w.pos == pos_person:
                e = u"?s :personName '{person}'." \
                    u"?s :hasActedIn ?m." \
                    u"?m :hasGenre ?g." \
                    u" ?g :genreName '{keyword}'".format(person=w.token, keyword=keyword)
    ...
    @staticmethod
    def is_actor_in_movie_question(word_objects):
        """
        3.1 某演员是某电影的演员吗
        :param word_objects:
        :return:
        ...
        e = u"?s :personName '{person}'." \
            u"?s :hasActedIn ?m." \
            u"?m :movieTitle '{movie}'.".format(person=person_1, movie=movie_1)
    ...
    def has_basic_person_info_question(word_objects):
        ...
        keyword = None
        for r in person_basic_keyword_rules:  # 匹配演员基本信息关键字
            keyword = r.apply(word_objects)
            if keyword is not None:
                break
...
mystery = (W("悬疑") | W("悬疑片"))
music = (W("音乐") | W("音乐片")| W("音乐剧"))
romance = (W("爱情") | W("爱情片")| W("罗曼蒂克"))
family = W("家庭")
war = (W("战争") | W("战争片"))
TV = W("电视")
...
actor = (W("演员") | W("艺人") | W("表演者")| W("主演"))
movie = (W("电影") | W("影片") | W("片子") | W("片") | W("剧"))
category = (W("类型") | W("种类"))
several = (W("多少") | W("几部"))

higher = (W("大于") | W("高于")| W("多于"))
lower = (W("小于") | W("低于")| W("少于"))
...
person_basic_info = (birth | birth_place | english_name | introduction)

rating = (W("评分") | W("分") | W("分数"))
release = (W("上映"))
movie_basic_info = (rating | introduction | release)
...
1.4.某演员出演的某类型电影有哪些？
1.5.某演员出演过哪些类型的电影？
1.6.某演员参演的评分大于某分数的电影有哪些？
1.7 某演员的生日/出生地/英文名/简介
1.8.某电影的上映日期/评分
2.1 某演员出演了多少部电影?
2.2 演员A和演员B合作过几部电影？
2.3 某演员演过几部某类型的电影？
...
rules = [
    ...
    Rule(condition_num=2, condition=(movie_entity + Star(Any(), greedy=False) + actor + Star(Any(), greedy=False)) | (Star(Any(), greedy=False) +actor + Star(Any(), greedy=False) + movie_entity + Star(Any(), greedy=False)), action=QuestionSet.has_actor_question),
    ...
    Rule(condition_num=3, condition= person_entity + Star(Any(), greedy=False) + genre |(genre +movie)+ Star(Any(), greedy=False), action=QuestionSet. has_specific_type_movie_question),
    Rule(condition_num=3, condition=person_entity + Star(Any(), greedy=False) + category + Star(Any(), greedy=False) + movie + Star(Any(), greedy=False), action=QuestionSet.has_movie_type_question),
    Rule(condition_num=5, condition=person_entity + Star(Any(), greedy=False) + (rating + compare)| compare + number_entity + Star(Any(), greedy=False) + movie + Star(Any(), greedy=False), action=QuestionSet.has_compare_question),
    Rule(condition_num=3, condition=(person_entity + Star(Any(), greedy=False) + when + Star(Any(), greedy=False)) | (person_entity + Star(Any(), greedy=False) + person_basic_info + Star(Any(), greedy=False)) | (person_entity + Star(Any(), greedy=False) + person_basic_info + where+ Star(Any(), greedy=False)), action=QuestionSet.has_basic_person_info_question),
    Rule(condition_num=2, condition=movie_entity + Star(Any(), greedy=False) + movie_basic_info + Star(Any(), greedy=False), action=QuestionSet.has_basic_movie_info_question),

    Rule(condition_num=3, condition=person_entity + Star(Any(), greedy=False) + several +  (movie | Star(Any(), greedy=False)), action=QuestionSet.has_quantity_question),
    Rule(condition_num=4, condition=person_entity + Star(Any(), greedy=False) + person_entity + Star(Any(), greedy=False) + several + (movie | Star(Any(), greedy=False)), action=QuestionSet.has_cooperation_num_question),
    Rule(condition_num=5, condition=person_entity + Star(Any(), greedy=False) + several+ (genre + category + Star(Any(), greedy=False) + (movie | Star(Any(), greedy=False)))| (genre+ Star(Any(), greedy=False)), action=QuestionSet. has_specific_type_movie_num_question),

    Rule(condition_num=3,condition=person_entity + Star(Any(), greedy=False) + movie_entity + Star(Any(), greedy=False) | (Star(Any(), greedy=False) + actor), action=QuestionSet.is_actor_in_movie_question),
    Rule(condition_num=3, condition=person_entity + Star(Any(), greedy=False) + person_entity + Star(Any(), greedy=False) + movie_entity + Star(Any(), greedy=False), action=QuestionSet.has_cooperation_ask_question),
    Rule(condition_num=4, condition=person_entity + Star(Any(), greedy=False) + (genre + category + Star(Any(), greedy=False) + movie) | (genre + Star(Any(), greedy=False))| (category + Star(Any(), greedy=False) + actor + Star(Any(), greedy=False)),action=QuestionSet.is_sometype_person_question)
